...
RUN echo "deb http://repos.zend.com/zend-server/5.1.0/deb server non-free" >> /etc/apt/sources.list.d/zend-server.list
RUN apt-get update && apt-get install -y curl libmysqlclient18 unzip git zend-server-php-5.3=5.1.0 && /usr/local/zend/bin/zendctl.sh stop
